<div>
    @if($pages == 0)
        <div class="shadow-sm w-75 rounded mx-auto mt-2 p-3">
            ...
            <div class="row">
                <div class="col">
                    <input wire:model.fill="discount" value="{{ $find_product_discount->discount.'%' }}" disabled
                           class="bg-gray-50 mb-3 transition border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-blue-500 block w-full ps-10 p-2.5 light:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-green-500 dark:focus:border-green-500">
                </div>

                <div class="row">
                    <div class="col">
                        <input wire:model.fill="result_discount"
                               value="{{ number_format($find_product_discount['result_discount'], 0) }}" disabled
                               class="bg-gray-50 mb-3 transition border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-blue-500 block w-full ps-10 p-2.5 light:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-green-500 dark:focus:border-green-500">
                    </div>

                    <div class="col">
                        <input wire:model.fill="vazn" value="{{ $find_product_discount->vazn }}" disabled disabled
                               class="bg-gray-50 mb-3 transition border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-blue-500 block w-full ps-10 p-2.5 light:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-green-500 dark:focus:border-green-500">
                    </div>
                </div>
            </div>
    ...
    @elseif($pages == 1)
        <form wire:submit="create" class="shadow-sm mx-auto mt-2 w-50 rounded p-3">
            <div>
                ...
                <input wire:model.fill="vazn" value="{{ $find_product_discount->vazn }}" hidden disabled>
                <input wire:model.fill="discount" value="{{ $find_product_discount->discount.'%' }}" disabled hidden>
                <input wire:model.fill="result_discount"
                       value="{{ number_format($find_product_discount['result_discount'], 0) }}" disabled hidden>
                <input type="number" placeholder="مبغ را وارد کنید" min="{{ $find_product_discount->result_discount }}"
                       class="bg-gray-50 mb-3 transition border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-blue-500 block w-full ps-10 p-2.5 light:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-green-500 dark:focus:border-green-500"
                       value="{{ $find_product_discount->result_discount }}">
                <button class="flex w-full
                items-end float-end justify-center rounded-md bg-green-600 px-5 py-2.5 text-center text-sm shadow-sm mt-3 font-medium text-white hover:bg-green-700 hover:shadow focus:outline-none focus:ring-4 focus:ring-green-700 transition-all"
                        type="submit">خرید
                </button>
