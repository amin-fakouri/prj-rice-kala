<div>
    <div data-aos="fade-up" style="padding: 10px;"
        ...
        <div class="card my_card shadow-sm mx-auto mt-2"
            ...
            @switch($find_product_discount_id->pic_url)
                @case($find_product_discount_id['pic_url'] == 1)
                    <img
            ...
            @endswitch
            <div class="card-body">
                <h6 class="card-title text-center">{{ $find_product_discount_id->name_pro.$find_product_discount_id->vazn }}</h6>
                <hr>
                <p class="card-tex line-through text-gray-400"
                   style="font-size: 14px">{{ number_format($find_product_discount_id->price, 0).'تومان' }}</p>
                <p class="card-text">
                    <strong>{{ number_format($find_product_discount_id->result_discount,0).'تومان' }}</strong>
                </p>

                @foreach($companys as $company)
                    @if($company->user_id === $find_product_discount_id->user_id)
                ...
                <p class="card-text"><b>{{ 'تاریخ:'.$find_product_discount_id->time_create_product }}</b></p>
                <p style="background-color: rgba(0,0,0,0.8); color: white; padding: 5px; width: 50px; text-align: center;"
                   class="rounded-2xl card-text mt-1 mb-1">{{ $find_product_discount_id->discount.'%' }}</p>
                ...
                <div>
                    <a href="{{ \Illuminate\Support\Facades\URL::signedRoute('buy_product_discount', ['product_id' => $find_product_discount_id->id]) }}"
